...
public class Getlink {
		...
		try{
			...
			while((line = check.readLine()) != null){
				...
				if(matcher.find()){
					...
					//System.out.println("http://www.asahi.com"+matcher.group(0).split("\"")[1]);
	...
	public static List<String> getlinkReuters(List<String> links,String ht,String newsDomain){
		//List<String> links = new ArrayList<String>();
		System.out.println("URL="+ht);
		try{
			URL url = new URL(ht);
			...
			String line;
			String patternString = "<a href=\"/article/.++";
			Pattern domainPattern = 
					Pattern.compile(patternString);
			System.out.println(domainPattern);
			//文字列にhttp://www.yomiuri.co.jp/newsdomain/.*が含まれているか確認
			int n=1;
			List<String> checkdouble = new ArrayList<>();
			while((line = check.readLine()) != null){
				Matcher matcher = domainPattern.matcher(line);
				if(matcher.find()){
					//System.out.println("match point = "+"http://www.asahi.com"+matcher.group(0).split("\"")[1]);
					//条件にマッチしたURL部分をリンクに追加
					//String[] 去除一个 两个是重复的 所以是20
					String suburl = "http://www.reuters.com"+matcher.group(0).split("\"")[1]; 
					if(!checkdouble.contains(suburl)){
						//System.out.println(n);
						System.out.println("http://www.reuters.com"+matcher.group(0).split("\"")[1]);
						links.add("http://www.reuters.com"+matcher.group(0).split("\"")[1]);
						checkdouble.add(suburl);
						n++;
					}
